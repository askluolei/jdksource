...
public abstract class AbstractQueuedSynchronizer
    ...
    /**
     * 队列的头节点，延时加载，只通过 setHead 方法修改
     * 如果头节点存证，它的 waitStatus 不会是 CANCELLED
     */
    private transient volatile Node head;

    /**
     * 队列的尾节点，延时加载，只会倍 enq 方法。添加新的等待节点的时候被修改
     */
    ...
    /**
     * 设置同步器的状态值Sets the value of synchronization state.
     * volatile 关键字 写直达
    ...
    /**
     * 节点入队，如果队列还没初始化，则进行初始化,建立头尾节点（就是一个新节点，头尾引用指向它，后面尾节点会指向加入的节点）
     * 返回的是前置节点
     ...
     * 为当前线程，使用给定模式（独占，共享），创建一个节点，并且入队
    ...
    private Node addWaiter(Node mode) {
        ...
        // 否则调用enq（入队，如果队列没初始化，则先初始化）
        enq(node);
        return node;
    }

    /**
     * 设置头节点，头节点不代表任何线程,没有前驱节点
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws == Node.SIGNAL)
            ...
            return true;
        if (ws > 0) {
            // 前一个节点如果是 > 0 ,那么从node，一直向前遍历，剔除 waitStatus > 0 的节点,这个返回的就是false了
            /*
            ...
            // 其他情况，就将前一个节点的状态设置为 SIGNAL,标识，本节点需要得到信号，但是暂时不要阻塞本线程，本线程将做最后一次尝试
            /*
     ...
     * 如果有可能，再最后一次尝试获取
     * 成功了就是返回
     * 失败了，就判断失败后是否需要阻塞，是就是阻塞并检查中断（检查操作在中断后面）
     * 如果是被中断激活的，返回true
     * 否则返回false（被上一个节点激活）
     */
    /**
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                // 如果node 的前一个节点就是头节点，就尝试获取一下锁，如果成功，就是将自己设置为头节点
                ...
                // 如果不是头节点，判断是否在获取失败的时候需要阻塞，如果需要就阻塞，不需要，继续循环
    ...
    /**
     * 在重入锁里面调用了这个 acquire 方法
     * 首先是调用子类的 tryAcquire 尝试获取锁，如果成功，就返回
     * 如果失败，先添加这个线程到队列 addWaiter，这个方法添加节点到队尾，如果队列没初始化，则先初始化
     * 然后 acquireQueued
     */
    /**
    ...
    /**
     * 释放
     * 先调用子类的 tryRelease ，如果子类返回true了（代表真的释放掉了）
     * 获取头节点，判断头节点的状态，如果不为0，就激活继承人
     * 注意，节点的状态默认就是0，当有节点等待的时候，就会设置他的上一个节点的状态为 SINGAL
     */
    /**
    ...
    /**
     * 判断一个节点是否在同步队列里面（注意队列有两个，同步队列（双向 prev,next），和 等待队列(单向 nextWatier)）
    ...
    /**
     * 将节点的等待状态设置为取消，如果设置成功，加入节点到同步队列中，返回true，否则返回false
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        /**
         * 判断是否有异常，如果有，继续尝试取消等待节点状态，如果失败，返回抛异常标记，尝试取消成功，返回重新设置中断状态标记，如果没有异常返回0
